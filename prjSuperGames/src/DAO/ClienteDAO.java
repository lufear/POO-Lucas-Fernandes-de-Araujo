...
public class ClienteDAO {
	...
	private final String INSERT = "INSERT INTO tbCliente (cpfCliente, nomeCliente, telCliente, emailCliente, loginCliente, senhaCliente) values (?,?,?,?,?,?)";
	private final String UPDATE = "UPDATE tbCliente SET cpfCliente =?, nomeCliente=?, telCliente=?, emailCliente=?, loginCliente=?, senhaCliente=? WHERE idCliente=?";
	private final String DELETE = "DELETE FROM tbCliente WHERE idCliente =?";
	private final String LIST = "SELECT idCliente, cpfCliente, nomeCliente, telCliente, emailCliente FROM tbCliente";
	private final String LISTBYID = "SELECT idCliente, cpfCliente, nomeCliente, telCliente, emailCliente FROM tbCliente WHERE idCliente=?";
	...
	public boolean cadastroCli(ModelCliente cliente) throws SQLException{
		if (cliente != null) {
			...
			try {
				...
				pstm.setString(5, cliente.getLogin());
	...
	public void atualizar(ModelCliente cliente) throws SQLException {
		if (cliente != null) {
			Connection conn = null;
			try {
				conn = Conexao.getConexao();
